...
export const ProductCard = (props: ProductCardProps) => {
  return (
    <AnimateOnView
      className={`box-border caret-transparent outline-[oklab(0.708_0_0_/_0.5)] transition-transform transition-opacity duration-500 ease-out transform hover:-translate-y-1 hover:shadow-lg will-change-transform ${props.hasTopMargin ? "mb-4" : ""}`}
    >
      <div className="box-border caret-transparent outline-[oklab(0.708_0_0_/_0.5)]">
        <div className="box-border caret-transparent gap-x-6 flex flex-col outline-[oklab(0.708_0_0_/_0.5)] gap-y-6 rounded-[14px] border-2 border-solid border-black/10">
          ...
          {/* expanded content area - will grow the card height when open */}
          <div
            className={`box-border caret-transparent overflow-hidden transition-[max-height,opacity,padding] duration-300 ease-out px-6 ${props.isOpen ? 'max-h-[600px] opacity-100 py-4' : 'max-h-0 opacity-0 py-0'}`}
          >
            {props.children}
          </div>
        </div>
